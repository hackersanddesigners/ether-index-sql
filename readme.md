...
a python plugin for [etherpad-lite](https://github.com/ether/etherpad-lite/wiki/How-to-list-all-pads), that reads from the SQL database etherpad-lite is running from, and make a one-page index list of all the visible pads, plus some useful details for each. pads can be set to not be part of the index, by adding a special keyword on the first line of the pad itself (see *create .env* and the `FILTER` key).
